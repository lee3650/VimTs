...
import { NORMAL_MODE } from './model/Vim'
...
const VimApp: React.FC<{ startingStr: string[] }> = ({ startingStr }) => {
  ...
  const checkKeyPress = useCallback(
    (event: any) => {
      ...
      setCommandCursorPos([
        output.commandCursorPos.row,
        output.commandCursorPos.col,
      ])
  ...
  function formatLine(line: string, index: number) {
    if (mode == VISUAL_MODE) {
      ...
      return (
        <p key={index}>
          {before}
          <span className="cursorChar">{insel}</span>
          {after}
        </p>
      )
    } else {
      if (index == cursorPos[0]) {
        ...
        if (cursorPart.length === 0) cursorPart = ' '
        return (
          <span key={index}>
            {line.slice(0, cursorPos[1])}
            <span className="cursorChar">{cursorPart}</span>
            {line.slice(cursorPos[1] + 1)}
            {'\n'}
          </span>
        )
      }
  ...
  function formatFooter() {
    if (mode != COMMAND_MODE) {
      ...
      let cursorPart = commandText.slice(
        commandCursorPos[1],
        commandCursorPos[1] + 1
      )
      if (cursorPart.length === 0) cursorPart = ' '
      return (
        <p key={-1}>
          :{commandText.slice(0, cursorPos[1])}
          <span className="cursorChar">{cursorPart}</span>
          {commandText.slice(cursorPos[1] + 1)}
        </p>
      )
    }
  }
